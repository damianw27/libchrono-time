...
describe('Generators', () => {
  describe('#getMilitaryTimes', () => {
    test('should generate times across hours and respect end minute for the last hour', () => {
      const result = getMilitaryTimes('08:30', '10:15', 30);

      expect(result).toEqual(['08:30', '09:00', '09:30', '10:00']);
    });

    test('should handle end minute = 0 by allowing exactly one time in the last hour (minute 00)', () => {
      const result = getMilitaryTimes('10:00', '10:00', 30);

      expect(result).toEqual(['10:00']);
    });

    test('should generate using a custom interval', () => {
      const result = getMilitaryTimes('08:00', '08:50', 20);

      expect(result).toEqual(['08:00', '08:20', '08:40']);
    });

    test('should pad hour/minute to HH:mm', () => {
  ...
  describe('#getMeridiemTimes', () => {
    test('should convert inputs to military, generate times, then convert each output back to meridiem', () => {
      const result = getMeridiemTimes('8:00 AM', '9:00 AM', 30);

      expect(result).toEqual(['08:00 AM', '08:30 AM', '09:00 AM']);
    });

    test('should use default interval=30', () => {
